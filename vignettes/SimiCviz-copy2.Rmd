...
SimiCviz easily easily handles `SimiCPipeline` outputs but also accepts GRN weights in a flexible long format. At minimum, you need a `data.frame` with the structure:
...
- `.csv` file with TF activity score matrix (cells × TFs)
- `.pickle` files with per-label matrices (cells × TFs) [Legacy Simiv v1 - Deprecated]

These outputs are organized in a directory set like this:
```
Project/
├── inputFiles/
│   ├── TF_list.csv
...
## Manual Loading
If you want more control over the process or have SimiC v1 run outputs then you can generate the SimiCvizExperiment or AUCprocessor objects loading separately the files.
...
The `cell_labels` map each cell id in your expression matrix or activity scores matrix to a condition/phenotype. Required for activity score calculation and AUC visualizations.
...
In many biological settings, we may want to know how the GRN activity changes in the cell population. For that we compute an activity score per driver gene (TF) and per cell. The details of the method to calculate are explained in the original [**SimiC**](https://www.nature.com/articles/s42003-022-03319-7.pdf) article by Peng *et al* puclished in  *Communications Biology* (2021).

Here we provide a series of wrapper functions to apply this method to `SimiCPipeline` or any GRN method output.
...
Once you have computed activity scores, let's visualize the results!

# Network Visualization

For visualization we need a `SimiCvizExperiment` object loaded with the results.

If you used `load_SimiCPipeline` of computed the activity scores with `calculate_activity_scores` you already have it.
```{r}
...
If you used other methods to calculate a cell specific activity score metrics with a different algorithm, you can import those as well in:
...
To assess the goodness of SimiC's fitted model of target expression we can plot the distribution adjusted R² values across all targets.
```{r}
...
*If you already filtered your weight matrix before creating the SimiCviz obejct you can skip this part*
```{r}
...
***Note:**: You can also save teh output and acces indivicual plots
...
plot_dissimilarity_heatmap(simic, 
                           ...
                           save = FALSE)
```
Sometimes these transcriptional dynamics are cell-type or cell-cluster specific. For that we might be interested in calculating the dissimilarity scores across the labels within specific cell clusters. For that, we can subset the `SimiCvizExperiment` object by the desired labels and then calculate the dissimilarity scores and plot the heatmap as shown before.
...
We can also select the labels accross which we want to compute the dissimilarity score with the argument `labels` and customize the color and sorting options.
```{r}
# For labels 0,2
plot_dissimilarity_heatmap(simic,
                            labels = c(0,2),
