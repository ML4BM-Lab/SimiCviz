...
#' @examples #' # Generate minimal example data
#' weights_list <- list("0" = as.data.frame(matrix(rnorm(90), nrow = 3, 
#'                              dimnames = list(paste0("TF", 1:3), 
#'                                              paste0("Gene", 1:30)))),
#'                      "1" = as.data.frame(matrix(rnorm(90), nrow = 3, 
...
#' @return An object of class \code{AUCProcessor} with slots for weights,
...
#'
#' @param weights list with weight matrices per label (if simic input) 
#'        or data.frame with columns: 
#'        tf, target, weight, label, tf (if data.frame input). 
#'        adj_p_val (optional, for p-value filtering 'qc_type' = "p_value").
#' @param expression expression matrix (cells x cells) or path to file.
#'        **IMPORTANT**: Expression must be in genes x cells format 
#'        (genes as rows, cells as columns), matching 
#'        Seurat/SingleCellExperiment conventions.
#' @param cell_labels data.frame with cell-to-label mapping
#' @param qc_type character type of quality metric. 
#'        Values: "adj_r2" or other column name (pvalue, adj_p_val, etc.) 
#'        provided in weights in data.frame format. If "adj_r2", 
#'        targets with R2 below threshold are filtered out. 
#'        If other (i.e. "p_value"), targets with qc_metric above threshold
#'        are filtered out.
#' @param qc_threshold threshold for filtering targets based on 
#'        qc_metric (default: 0.7 for R2).
#' @param adj_r2_list (optional) A list of R2 values per label per 
#'        Only used if weights are provided in list format 
#'        qc_type is "adj_r2". 
#' @param n_cores number of cores for parallelization (default: 1)
#' @param backend parallelization backend ("sequential", "multicore", 
#'        "multisession")
...
AUCProcessor <- function(weights,
  ...
  if (is.null(weights)) {
    ...
    if (inherits(weights, "list")) {
      ...
      if (!is.null(qc_type)){
        ...
        if (qc_type == "adj_r2") {
            ...
            weight_filtered <- weights_label_mat[,adj_r2_label >= qc_threshold, 
                                                  drop = FALSE]
        ...
        message("No quality_metric provided in weights list; 
                using all targets.")
      }
    ...
    if (!is.null(qc_type) && 
                qc_type != "adj_r2" && 
                qc_type %in% colnames(weight_df) ){
        message(sprintf("Filtering weights by %s < threshold %g", 
                        qc_type, qc_threshold))
        # Keep below threshold
        mask_pval <- which(weight_df[[qc_type]] <= qc_threshold) 
        weight_df <- weight_df[mask_pval, , drop = FALSE]
    } else if (!is.null(qc_type) && 
                qc_type == "adj_r2" && 
                "adj_r2" %in% colnames(weight_df)) {
        message("Filtering weights by adjusted R2 threshold...")
        mask_adj_r2 <- which(weight_df$adj_r2 >= qc_threshold)
        weight_df <- weight_df[mask_adj_r2, , drop = FALSE]
    } else {
        if (!is.null(qc_type)) {
          message(sprintf("No %s column found in weights data.frame; 
                          skipping filtering.",qc_type))
            }
        }
  }
  }
  ...
  if (is.null(cell_labels) || !is.data.frame(cell_labels)) {
      stop("`cell_labels` must be a data.frame with columns 
             'cell' and 'label'.")
  }
  ...
  expr_mat <- .ensure_genes_x_cells_format(expr_mat, cell_labels)
    ...
    if (length(common_cells) == 0) {
        stop(
            "No cells match between expression matrix (colnames) and 
            cell_labels. ",
            "Expression matrix columns must contain cell identifiers matching 
            cell_labels$cell."
        )
    }
    ...
    if (length(common_cells) < ncol(expr_mat)) {
        warning(sprintf(
            "%d cells in expression matrix not found in cell_labels. 
            Subsetting to matching cells.",
    ...
    if (length(missing_targets) > 0) {
        warning(sprintf(
            "Missing %d target genes in expression matrix. 
    ...
    if (length(missing_genes) > 0) {
        message(sprintf(
            ...
            Number of tfs: %d and Number of targets: %d ",
...
setMethod(
    ...
    cell_to_label <- stats::setNames(object@cell_labels$label,
                                     object@cell_labels$cell)
    
    # Extract subset of expression matrix (targets only), 
    # transpose to cells x targets
    expr_targets <- Matrix::t(object@expression[object@target_ids, ,
                               drop = FALSE])
    ...
    if (is.null(user_backend) || !(user_backend %in% 
                          c("sequential", "multicore", "multisession"))){
    ...
    # Bigger chunks reduce scheduler overhead; especially important
    # for SnowParam
    ...
    if (verbose) {
      ...
      message(sprintf("  Targets: %.0f%% of available", 
                         percent_of_target * 100))
      message(sprintf("  Labels: %d | Cells: %d", n_labels, n_cells))
      message(sprintf("  Backend requested: %s | backend used: %s | 
                         workers: %d",
                      object@parallel_params$backend, 
                      effective_backend, n_workers))
      if (effective_backend == "multisession") {
        message("  Note: multisession (SOCK) has worker 
    ...
    if (verbose) {
      message(sprintf("AUC computation completed in %.2f seconds!", 
                       as.numeric(elapsed)))
      message(sprintf("  Result: %d cells x %d TFs", 
                       nrow(auc_result), ncol(auc_result)))
      if (effective_backend == "multisession") {
        message("  Tip: if runtime is dominated by setup overhead, 
  ...
  function(object, format = "wide") {
    ...
    if (format == "wide") {
      ...
      result <- reshape2::melt(as.matrix(object@auc_results), 
                               varnames = c("cell", "tf"),
                               value.name = "score")
        label_map <- stats::setNames(object@cell_labels$label,
                                     object@cell_labels$cell)
...
.ensure_genes_x_cells_format <- function(expr_mat, cell_labels) {
    ...
    if (is.null(colnames(expr_mat))) {
        stop(
          "Expression matrix must have column names corresponding 
          to cell identifiers. ",
    ...
    if (n_matching_cells == 0) {
        ...
        if (!is.null(expr_rownames)) {
            ...
            if (n_rownames_match > n_matching_cells) {
                ...
                message("Transposing expression matrix to 
                ...
                stop(
                    "Expression matrix column names do not match any cell 
                    identifiers in cell_labels and no rownames were found",
                    sprintf("Expression has %d columns, 
                            cell_labels has %d cells.", 
                            n_cols, n_cells_expected))
            }
        } else {
            stop(
                "Expression matrix column/row names do not match any cell 
                identifiers in cell_labels. ",
            sprintf("Expression has %d columns, cell_labels has %d cells.",
                      n_cols, n_cells_expected),
            sprintf("First few column names: %s", paste(head(expr_colnames, 3), 
                    collapse = ", ")),
            sprintf("First few row names: %s", paste(head(expr_rownames, 3), 
                    collapse = ", ")),
            sprintf("First few cell IDs: %s", paste(head(cell_ids, 3),
                      collapse = ", ")))
        } 
    }
    ...
    # Return the expression matrix in genes x cells format subset 
    # to matching cells
...
.validate_auc_processor_inputs <- function(weights, expression, cell_labels) {
    ...
    if (!all(required_cols %in% colnames(weights))) {
      stop(sprintf("weights data.frame must contain columns: %s",
    ...
    if (!is.null(colnames(expression))) {
      ...
      stop("Expression matrix must have column names corresponding
            to cell identifiers.")
    }
...
#' @param weight_df weight data.frame subset for this cell's label 
...
.compute_cell_auc <- function(expr_row,
    ...
    for (tf in tfs) {
        ...
        target_norm_regulon  <- target_norm[match(targets_in_tf, 
                                                  names(target_norm))]
        # Normalize weights
